...
def remove_long_data(dataset: Dataset, max_seconds: Union[int, float] = 6):
...
def load_data() -> Tuple[list, list]:
    ...
    train_list = []
    valid_list = []
    stats_file = "stats_train_valid_{}.csv".format(suffix)
    ...
    for i, l in enumerate(args.languages):
        ...
        valid_ipa = load_dataset("json",
                                 ...
                                 split="train")
        train_data, valid_data = get_audio_dataset(l)
        ...
        train_data, valid_data = add_ipa_column(train_data, valid_data)
        if l == "en":
...
def get_audio_dataset(lang: str):
    if lang == "en":
        ...
        train_data = rename_column("text", "sentence")
...
def clean_tamil_data(train_data: Dataset, valid_data: Dataset) -> Tuple[Dataset, Dataset]:
    if lang == "ta":
...
def add_ipa_column(train_data: Dataset, valid_data: Dataset) -> Tuple[Dataset, Dataset]:
