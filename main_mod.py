...
if __name__ == "__main__":
    ...
    model = Wav2Vec2ForCTC.from_pretrained(
        ...
        attention_dropout=0.15,  # Aumenta a regularização (era 0.0)
        hidden_dropout=0.15,    # Aumenta a regularização (era 0.0)
        feat_proj_dropout=0.1,  # Adiciona regularização no projetor de features (era 0.0)
        mask_time_prob=0.075,   # Máscara temporal mais frequente (era 0.05)
        layerdrop=0.2,          # Aumenta a probabilidade de desativar camadas (era 0.1)
    ...
    # for layer in model.wav2vec2.encoder.layers[:6]:  # pra melhorar o  transfer learning
    #     for param in layer.parameters():
    #         param.requires_grad = False
    print("Feature extractor frozen")

    # Adicionar camadas extras após o encoder e antes da camada CTC
    model.classifier = nn.Sequential(
    nn.Linear(model.config.hidden_size, 512),  
    nn.ReLU(),                               
    nn.Linear(512, len(processor_ipa.tokenizer))  
)
    ...
    training_args = TrainingArguments(
        ...
        eval_steps=100,
        logging_steps=10,
        learning_rate=3e-4, #taxa de aprendizado menos (era 3e-4)
        warmup_steps=500, #aumento do warmup (era 500)
        save_total_limit=2,
    )
